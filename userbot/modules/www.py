...
async def pingme(pong):
    ...
    await pong.edit(f"〠 __Test__ **PING** __|━|⎆__ ヅ "
                    f"\n  ☞ `%sms` \n"
                    f"〠 __My__ **Raja** __|━|⎆__ ヅ "
                    f"\n  👉🏼 `{ALIVE_NAME}` \n" % (duration))


@register(outgoing=True, pattern="^.hping$")
async def pingme(pong):
    """ For .ping command, ping the userbot from any chat.  """
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("�🐍")
    await pong.edit("__**...�🐍Hydra�🐍...**__")
...
async def pingme(pong):
    ...
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("`.....🐍Hydra🐍.....`")
    await pong.edit("`�🐍`")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await pong.edit(f"•⎚• ⎆ __Hydra__ **Pong!**\n"
                    f"👉🏼  __Ping:__ "
                    f"`%sms` \n"
                    f"👉🏼  __Sisa Waktu:__ "
                    f"`{uptime}` \n" % (duration))
...
async def pingme(pong):
    ...
    duration = (end - start).microseconds / 1000
    await pong.edit(f"- Hydra -\n"
                    f"**👉🏼 sinyal  :** "
                    f"`%sms` \n"
                    f"**👉🏼 i'm online  :** "
                    f"`{uptime}` \n"
                    f"__|━|⎆__ **Raja  :** `{ALIVE_NAME}`" % (duration))
...
async def pingme(pong):
    ...
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("`.....👉🏼HYDRA USERBOT👈🏼.....`")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await pong.edit(f"**Hydra PONG**\n"
                    f"➥__Ping:__ "
                    f"`%sms` \n"
                    f"➥__Uptime:__ "
                    f"`{uptime}` \n" % (duration))
...
async def pingme(pong):
    ...
    await pong.edit("⎚⎆ __HYDRA__ **Test Ping!**\n`%sms`" % (duration))

CMD_HELP.update({
    "ping":
    "•CMD:`.ping` ; •CMD:`.hping` ; •CMD:`.xping` ; •CMD:`.fping`\
