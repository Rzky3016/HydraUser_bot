...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"**Raja ⚜{ALIVE_NAME} Sedang AFK!**",
    f"**Raja ⚜{ALIVE_NAME} Sedang AFK\n •Mohon Menunggu Hingga Dia Online Lagi**",
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**Raja `Sudah Tidak Afk Lagi`...**")
        time.sleep(3)
        await msg.delete()
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"**Hydra AFK**\n╭✠╼━━━━━━❖━━━━━━━✠╮\n{ALIVE_NAME} Afk :)...\nLama AFK : {afk_since}.\
                        \n𝘼𝙡𝙖𝙨𝙖𝙣: `{AFKREASON}`\n╰✠╼━━━━━━❖━━━━━━━✠╯")
                else:
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"**Hydra AFK**\n╭✠╼━━━━━━❖━━━━━━━✠╮\nMohon Maaf `{ALIVE_NAME}` Lagi Afk...\nLama AFK : {afk_since}.\
                        \n𝘼𝙡𝙖𝙨𝙖𝙣: `{AFKREASON}`\n╰✠╼━━━━━━❖━━━━━━━✠╯")
                else:
